...
async def auth_login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    role = 'admin' if user.email == 'churros@admin.com' else 'user'
    return TokenAndUserReturn(access_token=token, token_type="bearer", id=user.id, name=user.name, email=user.email, role=role)
