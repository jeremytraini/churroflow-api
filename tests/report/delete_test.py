...
def test_delete_valid():
    token =auth_register_v2("test_delete_valid@gmail.com", "abc123")["token"]
...
def test_delete_invalid_report_id_negative():
    token =auth_register_v2("test_delete_invalid_report_id_negative@gmail.com", "abc123")["token"]
    assert report_delete_v2(token, -1)['detail'] == "Report id cannot be less than 0"
    clear_v1(token)

def test_delete_invalid_report_id_not_found():
    token =auth_register_v2("test_delete_invalid_report_id_not_found@gmail.com", "abc123")["token"]
...
def test_delete_valid_upload_invalid_token():
    token =auth_register_v2("test_delete_valid_upload_invalid_token@gmail.com", "abc123")["token"]
    report_id = invoice_upload_text_v2(token, "invoice", VALID_INVOICE_TEXT)["report_id"]
...
def test_delete_invalid_not_owner():
    token =auth_register_v2("test_delete_invalid_not_owner@gmail.com", "abc123")["token"]
    token2 =auth_register_v2("test_delete_invalid_not_owner1@gmail.com", "abc123")["token"]
    report_id = invoice_upload_text_v2(token, "invoice", VALID_INVOICE_TEXT)["report_id"]
